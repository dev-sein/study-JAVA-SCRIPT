...
//slice(begin, end): 원하는 부분을 추출하기 위해 시작 인덱스(inclusive)와 마지막 인덱스(exclusive) 를 전달한다. 0,3 넣으면 0부터 2까지임
//slice(begin); begin 부터 마지막 인덱스까지 추출

console.log(datas.slice(0, 3)); //다른 어레이 객체가 만들어져서 반환되는 것임
console.log(datas.slice(4)); 

//forEach(callback) : 반복문
datas.forEach(function(data){console.log(data);});
//datas=this // 각각의 값을 리턴함 (콜백함수의 매개변수로 전달)
